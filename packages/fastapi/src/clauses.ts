...
export function deriveClauses(bp: BackendBlueprint): ContractClause[] {
  ...
  clauses.push(
    ...
    clause("database", "behavior", "database:get-db", "get_db yields from the module-default SessionLocal and always closes the session; session_dependency(factory) is an ordinary function that RETURNS a callable FastAPI dependency (a closure over factory that yields a Session and closes it) — it is NOT itself a generator, so session_dependency(factory) produces a callable, never a generator object.", "oracle", "function"),
  ...
  clauses.push(
    ...
    clause("app", "app", "app:engine-isolation", "When app.database exists, each create_app call creates one engine via create_engine_from_url(database_url) and one session factory, stores them on app.state, overrides get_db with the RETURNED dependency from session_dependency(factory), creates tables on startup against that engine, and disposes it on shutdown; with no database module yet the skeleton accepts database_url and wires nothing — create_app never raises because a dependency module is absent.", "oracle", "function"),
